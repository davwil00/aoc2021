...
import day18.Snailfish.SnailfishNumber.Direction.LEFT
import day18.Snailfish.SnailfishNumber.Direction.RIGHT
...
class Snailfish {
    ...
    fun addAndReduceList(snailfishNumbers: List<String>): SnailfishNumber {
        return snailfishNumbers.map { SnailfishNumber.fromString(it) }
            .reduce { first, second ->
                logger.debug("added = ${first + second}")
                reduce(first + second)
            }
    }
    ...
    fun reduce(snailfishNumber: SnailfishNumber): SnailfishNumber {
        while(snailfishNumber.explode() || snailfishNumber.split()) {
            logger.debug(snailfishNumber.toString())
        }
    ...
    fun findLargestMagnitude(snailfishNumbers: List<String>): Int {
        val first = snailfishNumbers.maxOf { snailfishNumber ->
            snailfishNumbers
                .filter { it != snailfishNumber }
                .maxOf { other ->
                    reduce(
                        SnailfishNumber.fromString(snailfishNumber) +
                        SnailfishNumber.fromString(other)
                    ).getMagnitude()
                }
        }
    ...
    class SnailfishNumber(private var parent: SnailfishNumber?) {
        ...
        fun getMagnitude(): Int {
            val left = if (leftValue != null) leftValue!! * 3 else leftNode!!.getMagnitude() * 3
            val right = if (rightValue != null) rightValue!! * 2 else rightNode!!.getMagnitude() * 2
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(Snailfish::class.java)
    }
}
...
fun main() {
    ...
    val result = snailfish.addAndReduceList(input)
    println("Magnitude of summed list is: ${result.getMagnitude()}")
    println("Maximum magnitude is: ${snailfish.findLargestMagnitude(input)}")
}
